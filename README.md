...
`emails_history.txt` only ever grows, so a naive "read the whole file" approach to a question like *"full picture on Delta"* gets slower forever as your archive ages. The project instructions below tell Claude to run an actual command-line search (`Select-String`/grep-style) against `emails_history.txt` for the name or domain, and only load the matched blocks into context — instead of reading the entire file with a file-read tool. This keeps person/company lookups fast regardless of how many years of mail you've accumulated.

**This depends on the chat having command-line/bash tool access to the connected folder.** If it doesn't, the instruction's fallback line means it still works correctly — Claude just reads the whole file directly instead, with no speed benefit for large archives. To confirm which one is actually happening: ask directly ("did you use a command-line search or read the whole file?"), or compare how long a person/company lookup takes against a large archive versus a plain summary request of similar output length. If there's no noticeable difference, the fast path isn't engaged, and command-line access needs to be added to the project/connector setup.
...
Use emails_history.txt whenever I ask about a specific person or company by name — including phrasings like "full picture on X," "everything from X," "history with X," or "how many times has X emailed me" — or whenever I explicitly ask you to look back further than 10 days, or the answer isn't found in the other two files. For these named-entity queries specifically: do NOT open emails_history.txt with a file-read tool. Instead, use your command-line/bash access to run an actual text search against it — e.g. Select-String -Path emails_history.txt -Pattern "<name>" -Context 5,5 on Windows, or an equivalent grep-style search — for the name, email address, or domain I gave you (case-insensitive, and try reasonable variants — e.g. "Delta" should also match "delta.com" or "Delta Air Lines"), and only load the matched blocks — including their surrounding "---" boundaries — into context for your answer. This keeps the response fast regardless of how large the archive has grown, since the whole file is never pulled into context, only the matches. Only fall back to reading the full file directly if command-line search access isn't available to you in this session, or a targeted search genuinely turns up nothing plausible and you need to double-check nothing was missed due to a naming mismatch.
...
**v2.7**
* Corrected the named-entity lookup instruction. It previously said to "search/filter" `emails_history.txt`, which is ambiguous about mechanism — a plain file-read tool satisfies that wording too, just by loading the whole file first and filtering mentally afterward, which defeats the entire point of avoiding a full read as the archive grows. The instruction now explicitly requires an actual command-line search (`Select-String`/grep-style) and only falls back to a direct file read if command-line access isn't available in that session.
